    ...
    <div className="bg-background text-foreground min-h-screen pb-16 font-sans select-none relative">
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        ...
        <div className="flex border-b border-zinc-800/80 gap-6 mb-8">
          ...
          <button className="pb-4 text-sm font-bold text-brand-neon relative">
        ...
        <div className="flex justify-between items-center mb-8 gap-4">
          <div className="relative max-w-sm flex-1">
            ...
            <input
              ...
              className="w-full bg-surface border border-zinc-800 focus:border-brand-emerald rounded-xl py-2.5 pl-10 pr-4 text-xs text-foreground placeholder-zinc-700 outline-none transition-all font-sans"
            />
          </div>

          <button
            onClick={() => setIsCartOpen(!isCartOpen)}
            className="bg-surface border border-zinc-800 hover:border-zinc-700 p-2.5 rounded-xl flex items-center gap-2 text-xs font-mono tracking-wider text-foreground cursor-pointer"
          >
            ...
            <div key={p.id} className="glass-panel rounded-2xl overflow-hidden flex flex-col justify-between group">
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-tiny text-zinc-500 leading-relaxed mt-1.5 mb-4 font-sans">{p.desc}</p>
                </div>
                <div>
                  <div className="flex justify-between items-center border-t border-zinc-800/60 pt-3 mt-auto">
                    <div>
                      <span className="text-micro font-mono text-zinc-500 block">HARGA KONSINYASI</span>
                      <span className="text-xs font-mono font-bold text-brand-neon">Rp {p.price.toLocaleString("id-ID")}</span>
                    </div>
                    <button
                      onClick={() => addToCart(p)}
                      className="bg-zinc-800 hover:bg-brand-emerald hover:text-black border border-zinc-700 hover:border-transparent text-zinc-300 font-mono text-micro font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer hover:glow-emerald"
                    >
                      ADD TO CART
                    </button>
                  </div>
          ...
          <div className="w-full max-w-sm glass-panel border-l border-zinc-800 h-full p-6 flex flex-col justify-between shadow-2xl relative">
            ...
            <div className="flex-1 overflow-y-auto pr-2 mt-8">
              <h3 className="text-sm font-mono text-zinc-400 uppercase tracking-wider mb-6 flex items-center gap-1.5">
                <ShoppingBag className="w-4 h-4 text-brand-neon" /> Shopping Cart
              </h3>
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4 font-sans">
                  {cart.map((item) => (
                    <div key={item.id} className="bg-surface-elevated border border-zinc-800/80 p-3 rounded-lg flex items-center justify-between gap-3 text-xs">
                      <div className="flex-1">
                        <h5 className="font-bold text-white leading-none mb-1">{item.name}</h5>
                        <span className="text-micro font-mono text-brand-neon">
            ...
            {cart.length > 0 && (
              <div className="border-t border-zinc-800/80 pt-4 mt-4 space-y-4">
                <div className="flex justify-between items-center text-xs font-mono">
                  ...
                  <span className="text-brand-neon font-bold text-sm">Rp {cartTotal.toLocaleString("id-ID")}</span>
                </div>
                <div className="bg-surface-hover border border-zinc-800 p-2.5 rounded text-micro font-mono text-zinc-500 uppercase text-center">
                  🔐 100% SECURE CASHLESS SYSTEM
                </div>
                <button
                  onClick={handleCheckout}
                  className="w-full bg-brand-emerald hover:bg-brand-emerald/90 text-black font-black py-4 rounded-xl text-xs flex items-center justify-center gap-2 uppercase tracking-wider font-mono transition-all glow-emerald cursor-pointer"
                >
