...
export default function AdminScanPage() {
  ...
  const [scanResult, setScanResult] = useState({
    status: "READY", // "READY", "SUCCESS", "FAILED"
    ticketId: "",
    details: null
  });
  ...
  return (
    ...
    >
    <ScanBarcode className="w-3.5 h-3.5 text-[#4edea3]" />
    ...
    <div className="mb-8">
    <h1 className="text-2xl font-black text-white">Live Ticket Scanner</h1>
    ...
    {/* Overlay reticles */}
    <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2 border-[#4edea3]" />
    <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2 border-[#4edea3]" />
    <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2 border-[#4edea3]" />
    <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2 border-[#4edea3]" />

    {/* Laser scan lines */}
    {scanning && (
      <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-transparent via-[#4edea3] to-transparent shadow-[0_0_15px_#4edea3] animate-bounce w-full" />
    )}

    {/* Simulator UI inside viewport */}
    {scanning ? (
      <div className="text-center font-mono space-y-2">
      <RefreshCw className="w-8 h-8 text-[#4edea3] animate-spin mx-auto" />
    ...
    className="bg-emerald-900/30 hover:bg-emerald-900 border border-emerald-500/20 hover:border-transparent text-emerald-400 hover:text-[#003824] font-mono font-bold text-xs py-2.5 rounded transition-all"
    >
    ...
    {scanResult.status === "SUCCESS" && (
      ...
      <div className="p-3.5 bg-emerald-950/30 border border-emerald-500/30 rounded-lg flex items-center gap-3 text-emerald-400">
      <CheckCircle className="w-5 h-5 shrink-0" />
      <div>
      <h4 className="text-xs font-bold uppercase tracking-wider font-mono">TICKET VALIDATED</h4>
      <p className="text-[10px] text-emerald-500/80 mt-0.5 leading-none">Gate A Check-In Permitted</p>
      </div>
      </div>
      ...
      <span className="text-[#4edea3] font-bold">{scanResult.ticketId}</span>
      </div>
      <div className="flex justify-between">
      ...
      <span className="text-[#4edea3]">{scanResult.details?.gate}</span>
      </div>
      </div>
      </div>
    )}
    ...
    {scanResult.status === "FAILED" && (
      ...
      <p className="text-[10px] text-red-500/80 mt-0.5 leading-none">Forfeited / Invalid Pass</p>
      </div>
      </div>

      <div className="bg-[#0e0e0e] border border-zinc-800/80 p-4 rounded-lg text-xs leading-relaxed text-zinc-400">
    ...
    className="flex-1 bg-[#0e0e0e] border border-zinc-800 focus:border-[#4edea3] rounded-lg px-3.5 py-2 text-xs text-white placeholder-zinc-700 outline-none transition-all uppercase font-mono"
    />
    <button
    type="submit"
    className="bg-zinc-800 hover:bg-[#4edea3] hover:text-[#003824] border border-zinc-700 text-zinc-300 font-mono font-bold text-xs px-4 py-2 rounded-lg transition-all"
    >
    VALIDATE CODE
    </button>
    </form>
    ...
    {recentScans.map((log, i) => (
      <div key={i} className="flex justify-between items-center bg-[#0e0e0e] border border-zinc-900 p-2.5 rounded">
      <div>
      <span className="text-white font-bold">{log.ticketId}</span>
      <span className="text-zinc-500 text-[10px] ml-2 font-sans">{log.name}</span>
      </div>
      <div className="text-right">
      <span className="text-[#4edea3] text-[10px] block font-bold">{log.status}</span>
