  ...
  return (
    <div className="bg-background text-foreground min-h-screen pb-16 font-sans select-none">
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-foreground font-display">Court Slot Control Matrix</h1>
          <p className="text-zinc-400 text-xs md:text-sm mt-1">
            Konfigurator taktis dan pengontrol status lapangan futsal elite. Otoritas penuh untuk mengubah status status secara instan.
          </p>
        </div>

        {/* Date Row Selection */}
        <div className="flex gap-2 bg-surface border border-zinc-800/80 p-3 rounded-lg mb-8 max-w-sm">
          {dates.map((d) => (
            <button
              key={d.label}
              ...
              className={`flex-1 py-2 rounded font-mono text-xs font-bold transition-all ${
                selectedDate === d.label
                  ? "bg-zinc-800 border border-zinc-700 text-brand-neon"
          ...
          <div className="lg:col-span-8 bg-surface border border-zinc-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
              {slots.map((s) => (
                <div 
                  key={s.id}
                  className="bg-surface-elevated border border-zinc-800/60 rounded-lg p-3.5 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 hover:border-zinc-700 transition-all"
                >
                  <div className="flex items-center gap-4">
                    <span className="font-mono text-base font-bold text-foreground w-14">
                      {s.time}
                    </span>
                    <div className="flex items-center gap-2">
                      <span className={`w-2.5 h-2.5 rounded-full ${
                        s.state === "AVAILABLE" ? "bg-brand-emerald" :
                        s.state === "LOCKED" ? "bg-brand-amber animate-pulse" : "bg-zinc-500"
                      }`} />
                  ...
                  <div className="flex items-center gap-2.5 w-full md:w-auto justify-end">
                    ...
                    {s.state !== "BOOKED" && (
                      <button
                        ...
                        className="bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-foreground border border-zinc-800 px-3 py-1.5 rounded text-micro font-mono cursor-pointer transition-all"
                      >
                ...
                <div className="space-y-4">
                  <div className="bg-surface-elevated border border-zinc-800/80 p-4 rounded-lg font-mono text-xs space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="text-brand-neon font-bold">{selectedAuditSlot.time}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-500">ATHLETE</span>
                      <span className="text-foreground">{selectedAuditSlot.customer}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-500">PHONE</span>
                      <span className="text-foreground">{selectedAuditSlot.phone}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-500">TICKET INVOICE</span>
                      <span className="text-brand-amber font-bold">{selectedAuditSlot.payment}</span>
                    </div>
                  </div>

                  <div className="p-3 bg-amber-950/20 border border-amber-500/20 rounded text-tiny text-zinc-400 leading-normal">
                  ...
                  <button
                    ...
                    className="w-full bg-red-900/30 hover:bg-red-900 border border-red-500/30 hover:border-transparent text-red-400 hover:text-foreground font-mono font-bold text-xs py-2.5 rounded transition-all"
                  >
