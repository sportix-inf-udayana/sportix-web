 ...
 * Path: src/app/(dashboard)/seller-umkm/orders/page.js
 * Deskripsi SRS: 
 * Antarmuka manajemen siklus hidup penjualan produk UMKM. Digunakan untuk memantau invoice pembelian dari konsumen, 
 * mencetak manifest pengemasan barang, mengonfirmasi penjualan item dari pro-shop, serta melakukan input resi kurir pengiriman.
 */
  ...
  return (
    <div className="bg-[#09090b] text-[#e5e2e1] min-h-screen pb-16 font-sans select-none">
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-white">Consignment Shipment Tracker</h1>
          ...
          <div className="lg:col-span-8 bg-[#131313] border border-zinc-800 rounded-xl p-6">
            ...
            <div className="space-y-4">
              {orders.map((order) => (
                <div 
                  key={order.id}
                  className="bg-[#0e0e0e] border border-zinc-800/80 p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-zinc-700 transition-all"
                >
                  <div>
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-[10px] font-mono text-zinc-500">{order.date}</span>
                      <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded uppercase ${
                        order.status === "PENDING" ? "bg-amber-500/15 text-amber-400 border border-amber-500/20" : "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20"
                      }`}>
                        {order.status}
                      </span>
                    </div>
                  ...
                  <div className="text-right flex flex-col items-end border-t md:border-t-0 border-zinc-800 pt-3 md:pt-0 w-full md:w-auto">
                    <span className="text-[9px] font-mono text-zinc-500 block uppercase">LOCAL COURIER</span>
                    ...
                    <button 
                      ...
                      className="text-[10px] font-mono text-purple-400 hover:underline mt-2 cursor-pointer"
                    >
                      MANAGE SHIPMENT
                    </button>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-[#131313] border border-zinc-800 rounded-xl p-6">
              ...
              <form onSubmit={handleUpdateCourier} className="space-y-4 text-xs font-mono">
                <div>
                  <label className="text-zinc-500 block mb-1 uppercase text-[10px]">
                    SELECT ORDER ID
                  </label>
                  <select
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0e0e0e] border border-zinc-800 focus:border-purple-500 rounded-lg py-2.5 px-3 text-[#e5e2e1] outline-none placeholder-zinc-700 font-sans"
                  />
                </div>
