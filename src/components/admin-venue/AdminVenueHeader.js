// src/components/admin-venue/AdminVenueHeader.js
"use client";
import React, { useEffect, useState } from "react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import { Gauge, Clock, ShieldCheck, LogOut, User, BarChart3 } from "lucide-react";
...
export default function AdminVenueHeader() {
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
          <div className="flex bg-zinc-900 border border-zinc-800 p-1.5 rounded-xl w-full sm:w-auto overflow-x-auto scrollbar-none">
            ...
            >
              <ShieldCheck className={cn("w-4 h-4", pathname.includes('/scan') && "text-blue-400")} />
              <span>SCANNER</span>
            </Link>
            <Link
            ...
            >
              <BarChart3 className={cn("w-4 h-4", pathname.includes('/reports') && "text-blue-400")} />
              <span>REPORTS</span>
            </Link>
          </div>
