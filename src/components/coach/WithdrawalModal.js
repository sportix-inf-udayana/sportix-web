// src/components/coach/WithdrawalModal.js
"use client";
...
export default function WithdrawalModal({ isOpen, onClose, availableBalance }) {
  ...
  const withdrawalSchema = z.object({
    amount: z.coerce
      .number({ invalid_type_error: "Nominal tidak valid" })
      .min(50000, "Minimal Rp 50.000")
      .max(availableBalance, "Saldo tidak cukup"),
    bank_name: z.string().min(2, "Wajib diisi"),
    account_number: z.string().min(5, "Rekening tidak valid"),
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error?.message || result.message || "Gagal memproses penarikan.");
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col relative slide-in-from-bottom-4">
        <div className="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-900/50">
          ...
          <button
            ...
            className="text-zinc-500 hover:text-white bg-zinc-900 hover:bg-zinc-800 p-2 rounded-lg transition-colors disabled:opacity-50 cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-6">
          {success ? (
            <div className="py-12 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-lg font-bold text-white mb-2 font-display uppercase tracking-wide">Permintaan Diproses</h4>
              <p className="text-xs text-zinc-400 font-mono">Dana akan ditransfer dalam 1x24 jam operasional.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              {submitError && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start gap-2 text-red-400 font-mono text-[10px] uppercase tracking-widest">
              ...
              <div className="pt-4 border-t border-zinc-800/60 flex items-center gap-4">
                <button
                  type="button"
                  onClick={handleClose}
                  disabled={isSubmitting}
